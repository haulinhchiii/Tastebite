const CategoryRecipes = {
  ...
  async mounted() {
    try {
      const response = await fetch(`resources/api_recipe.php/category/${this.categoryId}`);
  ...
  template: `
    <div>
      <app-nav></app-nav>
      <div class="container mt-5 mb-5" v-boldheading style="margin-top: 120px !important;">
        <div class="row mb-4">
          <div class="col">
            <h1 class="text-start">Recipes for {{ categoryName }}</h1>
            <hr>
          </div>
        </div>
